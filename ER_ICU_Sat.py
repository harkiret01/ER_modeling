...
def simulation(p, t):
   # inflow
   pin = 2  # patient/hr into ER

   # constants
   c1 = 0.1 #Covid patients into ICU
   c2 = 0.6 #Covid death rate out of ICU
   
   #ECU
   nurses = 5 #1 nurse = 4 patients
   ventilators = 10 # 1 ventilator = 1 patient
   ctscan = 2 # 1 CT-Scan = 5 patients
   
   #ECU capacity
   ecu_capacity = min(4*nurses, ventilators, 5*ctscan)
   
   #ICU
   doctors = 2 # 1 doctor = 8 patients
   respirators = 2 # 2 respirator = 1 patient
   beds = 8 # 1 bed = 1 patient
